...
```bash
uv run python .Codex/skills/zulip-mark-feedback/scripts/pull_submissions.py \
  --week N \
...
1. List the feedback files; offer a `--dry-run` first:
   ...
   ```bash
   uv run python .Codex/skills/zulip-mark-feedback/scripts/send_feedback.py \
     --week N --dry-run
   ```
...
3. Execute the chosen action:
   ...
   ```bash
   # Option A — send all
   uv run python .Codex/skills/zulip-mark-feedback/scripts/send_feedback.py --week N
