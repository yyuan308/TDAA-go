...
Pick one set per course (read `textbook/*.md` and `weekN/plan.md` to decide — if the source material proves things from axioms, use theorem/proof; if it states laws and derives consequences, use keyresult/derivation). The "Proof Quality" criteria in `AGENTS.md` apply identically to `#derivation` — read *key insight* as *physical insight*, *why non-trivial* as *what naïve picture this overturns*.
