...
```bash
uv run python .Codex/skills/setup-zulip-grading/scripts/verify.py --config .zuliprc
```
...
```bash
ZULIP_API_KEY="<pasted>" uv run python .Codex/skills/setup-zulip-grading/scripts/create_bot.py \
  --site "<realm_url>" \
